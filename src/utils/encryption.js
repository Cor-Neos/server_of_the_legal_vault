...
if (!MASTER_KEY_HEX) {
  console.warn('[encryption] MASTER_KEY env var not set. Generate one (32 bytes hex) for production.');
}
const masterKey = MASTER_KEY_HEX ? Buffer.from(MASTER_KEY_HEX, 'hex') : null;

function ensureMasterKey() {
  if (!masterKey || masterKey.length !== 32) {
    throw new Error('Master key missing or invalid length. Set MASTER_KEY to 64 hex chars.');
  }
}
...
export function wrapKey(plaintextKey) {
  ...
  return { encKey: enc.toString('base64'), wrapIV: iv.toString('base64'), wrapTag: tag.toString('base64') };
}
...
export function decryptToStream({ encryptedPath, metadata, writable }) {
  ...
  const decipher = createDecipheriv('aes-256-gcm', dk, Buffer.from(iv, 'base64'));
